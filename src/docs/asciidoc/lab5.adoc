...
To open a window that runs the following Java command I clicked `http-source-rabbit.sh`.

----
include::{projectdir}/files/http-source-rabbit.sh[]
----

== Step 3 - Start log-sink-rabbit

I also clicked `http-source-rabbit.sh` that opened another windows running the following command:

----
include::{projectdir}/files/log-sink-rabbit.sh[]
----
...
I noticed the following output on `log-sink-rabbit` window:
...
In this lab, I basically review things did behind the scenes by the previous labs.
There where two microservices that run independently talking to each other through a message broker (RabbitMQ).
...
In my case, when some input is available on the message broker, it will be consumed (removed from the "channel") and presented in the log output of the microservice.
...
So, both `http-source` and `log-sink` can be easily scaled up!
