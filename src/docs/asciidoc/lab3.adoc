...
I stopped the rabbitmq container:
...
I left the rabbitmq container running to execute my next steps.
...
dataflow:>app register --type source --name http --uri file:///Users/pj/labs/spring-cloud-dataflow/http-source-rabbit-1.3.1.RELEASE.jar
...
I exited the spring-cloud-dataflow shell:
...
I started `tail` to monitoring the `stdout_0.log` file:
...
*** I needed to modify my `pom.xml` in order to make `Transformer` test execute.
I found https://stackoverflow.com/a/42237483[a solution on Stack Overflow].
